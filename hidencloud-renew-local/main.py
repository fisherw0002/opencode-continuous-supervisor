...
# ================= 配置常量 =================
RENEW_DAYS = 7
CACHE_FILE_NAME = 'hiden_cookies_US.json'
...
class HidenCloudBot:
    def __init__(self, env_cookie, index):
        ...
        self.csrf_token = ""
        self.services = []
        raw_target_ids = os.environ.get('TARGET_SERVICE_IDS', '').strip()
        self.target_service_ids = {x.strip() for x in raw_target_ids.split(',') if x.strip()} if raw_target_ids else set()
        ...
        try:
            ...
            for a in soup.find_all('a', href=True):
                ...
                if '/service/' in href and '/manage' in href:
                    ...
                    if self.target_service_ids and svc_id not in self.target_service_ids:
                        continue
...
if __name__ == '__main__':
    ...
    final_content = "\n".join(ALL_LOGS)
    if final_content:
        send_notify("HidenCloud (US) 续期报告", final_content)
